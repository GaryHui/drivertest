...
{t:'在高速公路违法占用应急车道行驶，一次记多少分？',a:['3分','6分','9分','12分'],c:1,e:'记6分。容易与“高速违法停车9分”混淆。',topic:'记分题',number:true,movieMemory:true},
...
{t:'机动车在高速公路违法停车，一次记6分。',a:['正确','错误'],c:1,e:'错误，高速违法停车记9分；违法占用应急车道行驶才是6分。',topic:'记分题',number:true,movieMemory:true}
];

// 一条规则至少对应一道题，保证7张表与练习题库同步覆盖。
const allCards=topics.flatMap(t=>t.cards.map((card,index)=>({topic:t.name,card,index})));
const memoryTips={'信息变更':'📱 换电话像报平安：30天内告诉车管所。','换证窗口':'📅 到期是大事，提前一个季度：90天。','实习期':'🌱 新手要走完一整年：12个月。','满分学习·小型车':'🪜 小车按周爬：7起、加7、60封顶。','满分学习·重点车':'🏋️ 大车按月扛：30起、加30、120封顶。'};
allCards.forEach((item,idx)=>{
  ...
  questions.push({
    ...
    topic:item.topic,number:/\d/.test(correct),newreg:['记分题','时间题','证件题'].includes(item.topic),generated:true,rule:`${topics.find(t=>t.name===item.topic).id}-${item.index}`,
    breakdown:{who:item.card[1],when:item.card[2],number:item.card[3],limit:item.card[4],mix:item.card[5]},memory:memoryTips[item.card[0]]
  });
});

questions[0].newreg=true;
questions[0].breakdown={who:'小型载客汽车驾驶人',when:'一个记分周期内，每多一次满分教育或累计每增加12分',number:'学习增加7天',limit:'每次最多60天',mix:'重点大车是30天起、加30天、最多120天'};
questions.push(
{t:'机动车驾驶人联系电话、联系地址等信息发生变化的，应当在信息变更后三十日内，向驾驶证核发地车辆管理所备案。',a:['正确','错误'],c:0,e:'正确。题干关键词是“联系电话、联系地址等信息变化”，法定期限为信息变更后30日内备案。',topic:'新规题',newreg:true,number:true,memory:'📱 换电话像报平安：30天内告诉车管所。',breakdown:{who:'机动车驾驶人',when:'联系电话、联系地址等信息发生变化',number:'30日内',limit:'向驾驶证核发地车辆管理所备案',mix:'驾驶证到期换证是提前90日'}},
{t:'机动车驾驶人在一个记分周期内有两起以上违法行为应当记分的，记分分值如何计算？',a:['只记最高一项','取平均分','累积计算','只记最早一项'],c:2,e:'第163号令第十四条：两起以上应记分的违法行为，分值累积计算。',topic:'新规题',newreg:true,breakdown:{who:'机动车驾驶人',when:'同一记分周期有两起以上应记分违法',number:'分值累积计算',limit:'满12分后不得再处理其他机动车违法记录',mix:'不是只取最高一项'}},
{t:'一个记分周期内二次累积满12分，或者累计满24分未满36分，科目一合格后还要考什么？',a:['不再考试','科目二','科目三道路驾驶技能','科目二和科目三'],c:2,e:'二次满12或24—35分：科目一合格后再考科目三。',topic:'新规题',newreg:true,breakdown:{who:'满分驾驶人',when:'二次满12分，或累计24—35分',number:'加考科目三',limit:'考试不合格10日后重约',mix:'达到36分才是科目二＋科目三'}},
{t:'一个记分周期内三次以上累积满12分，或者累计记分满36分，应加考哪些科目？',a:['只考科目一','只考科目三','科目一合格后考科目二和科目三','只考科目二'],c:2,e:'三次以上或达到36分：科目一合格后，再考科目二和科目三。',topic:'新规题',newreg:true,breakdown:{who:'满分驾驶人',when:'三次以上满12分，或累计达到36分',number:'科目二＋科目三',limit:'不合格10日后重约',mix:'24—35分只加考科目三'}},
{t:'参加交通安全教育减免交通违法记分，一个记分周期最多可以扣减多少分？',a:['3分','6分','9分','12分'],c:1,e:'学法减分一个记分周期累计最高扣减6分，不是给驾驶证增加6分。',topic:'新规题',newreg:true,number:true,breakdown:{who:'累积记分未满12分且符合条件的驾驶人',when:'参加交通安全教育达到要求',number:'最高扣减6分',limit:'一个记分周期内',mix:'是减现有记分，不是把驾驶证变成18分'}},
{t:'网上参加学法减分学习，三日内累计学习多久且考试合格，一次扣减1分？',a:['10分钟','20分钟','30分钟','60分钟'],c:2,e:'三日内网上学习累计满30分钟且考试合格，一次扣减1分。',topic:'新规题',newreg:true,number:true,breakdown:{who:'申请学法减分的驾驶人',when:'网上学习并考试合格',number:'3日内累计30分钟，减1分',limit:'周期累计最多减6分',mix:'现场学习满1小时且考试合格，一次减2分'}},
{t:'驾驶人参加满分学习考试时弄虚作假，除成绩无效外，处多少罚款？',a:['五百元以下','一千元以下','二千元以下','五千元以下'],c:1,e:'本人在满分教育或考试中弄虚作假：一千元以下罚款。',topic:'新规题',newreg:true,number:true,breakdown:{who:'参加满分教育的驾驶人本人',when:'签注学习记录或考试中弄虚作假',number:'1000元以下',limit:'学习记录、考试成绩同时无效',mix:'代替别人参加是2000元以下'}},
{t:'代替实际驾驶人参加满分教育、满分考试或学法减分的，处多少罚款？',a:['五百元以下','一千元以下','二千元以下','五千元以下'],c:2,e:'冒名代替别人参加学习或考试：二千元以下罚款。',topic:'新规题',newreg:true,number:true,breakdown:{who:'冒名顶替者',when:'代替实际驾驶人参加满分教育、考试或学法减分',number:'2000元以下',limit:'相关记录或成绩无效',mix:'驾驶人本人作弊是1000元以下'}},
{t:'驾驶机动车拨打、接听手持电话等妨碍安全驾驶，一次记几分？',a:['1分','2分','3分','6分'],c:2,e:'现行规则记3分；旧题库中的2分已经取消。',topic:'新规题',newreg:true,number:true,breakdown:{who:'机动车驾驶人',when:'驾驶时拨打、接听手持电话',number:'3分',limit:'停车并确保安全后使用不属于驾驶中',mix:'旧规定曾记2分，现行没有2分档'}},
{t:'驾驶机动车在高速公路或者城市快速路上违法停车，一次记几分？',a:['3分','6分','9分','12分'],c:2,e:'高速或城市快速路违法停车记9分。占应急车道行驶才是6分。',topic:'新规题',newreg:true,number:true,movieMemory:true,breakdown:{who:'机动车驾驶人',when:'高速公路或城市快速路上违法停车',number:'9分',limit:'高速公路、城市快速路都一样',mix:'占用应急车道行驶是6分'}},
{t:'代替实际机动车驾驶人接受处罚和记分并牟取经济利益，一次记多少分？',a:['6分','9分','12分','不记分'],c:2,e:'替人销分并牟利，一次记12分。',topic:'新规题',newreg:true,number:true,breakdown:{who:'替实际驾驶人接受处罚和记分的人',when:'替人销分并牟取经济利益',number:'12分',limit:'另处违法所得3倍以下、最高5万元罚款',mix:'请他人代扣分的一方也会被罚，但本题问替扣牟利者的记分'}}
);
...
function startQuiz(){let pool=quizMode==='wrong'?questions.filter((_,i)=>state.wrong.includes(i)):quizMode==='numbers'?questions.filter(q=>q.number):quizMode==='newreg'?questions.filter(q=>q.newreg&&!q.generated):quizMode==='official'?officialQuestions:questions.filter(q=>!q.generated);if(!pool.length){quizBox.innerHTML=`<div class="empty"><h3>${quizMode==='official'?'本地题库正在载入':'错题本还是空的 🎉'}</h3><p>${quizMode==='official'?'请稍候再试。':'先完成一轮摸底，答错的题会自动来到这里。'}</p></div>`;return}quiz=[...pool].sort(()=>Math.random()-.5).slice(0,10);qIndex=0;renderQuestion()}
function renderQuestion(){locked=false;if(qIndex>=quiz.length){const score=quiz.filter(q=>q._ok).length;quizBox.innerHTML=`<div class="empty"><p class="eyebrow">本轮完成</p><h3>${score===quiz.length?'满分通关！':'答对 '+score+' / '+quiz.length}</h3><p>${score/quiz.length>=.9?'已经达到90分合格线，继续保持。':'回到错题急救，把混淆数字再对比一次。'}</p><button class="primary" onclick="startQuiz()">再来一轮</button></div>`;return}const q=quiz[qIndex];quizBox.innerHTML=`<div class="quiz-progress"><i style="width:${qIndex/quiz.length*100}%"></i></div><div class="quiz-meta"><span>${q.topic}</span><b>${qIndex+1} / ${quiz.length}</b></div><h3>${q.t}</h3>${q.image?`<img class="question-image" src="${q.image.replace('http://','https://')}" alt="题目配图">`:''}<div class="answers">${q.a.map((a,i)=>`<button class="answer" data-a="${i}"><b>${String.fromCharCode(65+i)}.</b> ${a}</button>`).join('')}</div><div id="feedback"></div>`;quizBox.querySelectorAll('.answer').forEach(b=>b.onclick=()=>answer(+b.dataset.a))}
const escapeHtml=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
function memoryScene(stem){const clean=stem.replace(/[，。？！、；：,.!?]/g,' ').replace(/(下列|以下|关于|正确|错误|应当|可以|不得|一次|多少|如何|什么|的是|为)/g,' ').split(/\s+/).filter(x=>x.length>1);return (clean.sort((a,b)=>b.length-a.length)[0]||stem).slice(0,18)}
function memoryHook(q){const s=q.t,answer=q.a[q.c];const hooks=[
  [/高速公路.*违法停车|城市快速路.*违法停车/,'🚗🛑','车停住赖很“久”——高速违法停车9分。','溜六，久九'],
  [/应急车道.*(行驶|占用)/,'🚗💨','车还在应急道上“溜”——占道行驶6分。','溜六，久九'],
  [/能见度.*200/,'🌫️','看见200米：速度60、距离100。','二六一百'],
  [/能见度.*100/,'🌫️','看见100米：速度40、距离50。','一四五十'],
  [/能见度.*50/,'🌫️','只看50米：速度20，尽快驶离。','五二离开'],
  [/信息.*变更|联系地址|联系电话/,'📱','换电话像报平安，30日内告诉车管所。','变化三十'],
  [/有效期.*换证|期满换证/,'📅','证件快到期，提前一个季度准备。','到期九十'],
  [/高速公路.*故障.*警告|警告标志.*高速/,'🛑','高速车快，警告牌要放到150米以外。','高速一百五'],
  [/公交站|急救站|加油站|消防站/,'🚌','有“站”就记30米，站三。','站三口五'],
  [/路口|急弯|窄路|桥梁/,'↪️','路口弯桥范围更大，记50米。','站三口五'],
  [/饮酒.*驾驶/,'🍺','喝了是暂扣6个月；醉了直接吊销。','喝暂醉吊'],
  [/醉酒.*驾驶/,'🥴','醉到失控，驾驶证直接吊销。','喝暂醉吊'],
  [/吊销|撤销/,'✂️','开车违法叫吊销，拿证作假叫撤销。','开吊证撤'],
  [/12分/,'🕛','最危险的一档像时钟敲满12下。','大危险，满12'],
  [/灯光|远光灯|近光灯/,'💡','先看会不会晃到别人：会晃眼就用近光。','见人近，没人远']
 ];const hit=hooks.find(([re])=>re.test(s));return hit?{icon:hit[1],story:hit[2],chant:hit[3]}:{icon:/\d/.test(answer)?'🔢':'🎯',story:`看到“${memoryScene(s)}”，马上锁定“${answer}”。`,chant:`${memoryScene(s)} → ${answer}`}}
function buildWrongMemory(q,choice){const right=escapeHtml(q.a[q.c]),wrong=escapeHtml(q.a[choice]),scene=escapeHtml(q.breakdown?.when||memoryScene(q.t)),hook=memoryHook(q),mix=escapeHtml(q.breakdown?.mix||`不要再选“${q.a[choice]}”`);return `<div class="all-memory"><div class="memory-head"><span>${hook.icon}</span><div><small>这题只记一个画面</small><b>${scene}</b></div></div><div class="choice-swap"><span><small>刚才选成</small><del>${wrong}</del></span><i>→</i><span class="right-answer"><small>看到题眼就选</small><strong>${right}</strong></span></div><p class="memory-story">${escapeHtml(hook.story)}</p><div class="memory-chant">口令：<b>${escapeHtml(hook.chant)}</b></div><p class="memory-mix">别串台：${mix}</p><button class="recall-btn" type="button">🙈 遮住答案，自己说一遍</button></div>`}
function answer(choice){if(locked)return;locked=true;const q=quiz[qIndex],ok=choice===q.c,globalIndex=questions.indexOf(q);q._ok=ok;state.answered++;if(ok){state.correct++;if(globalIndex>=0)state.wrong=state.wrong.filter(i=>i!==globalIndex)}else if(globalIndex>=0&&!state.wrong.includes(globalIndex))state.wrong.push(globalIndex);save();quizBox.querySelectorAll('.answer').forEach((b,i)=>{if(i===q.c)b.classList.add('correct');else if(i===choice)b.classList.add('wrong');b.disabled=true});const memory=q.memory?`<div class="memory-tip">${q.memory}</div>`:'';feedback.innerHTML=`<div class="explain"><b>${ok?'✓ 判断正确':'✕ 这题混淆了'}</b><br>${q.e}</div>${ok?memory:buildWrongMemory(q,choice)}<button class="next-btn">下一题 →</button>`;const recall=feedback.querySelector('.recall-btn');if(recall)recall.onclick=()=>{const card=recall.closest('.all-memory');card.classList.toggle('recalling');recall.textContent=card.classList.contains('recalling')?'👀 显示答案':'🙈 遮住答案，自己说一遍'};feedback.querySelector('.next-btn').onclick=()=>{qIndex++;renderQuestion()}}
...
async function loadOfficialBank(){try{const data=await fetch('data/question-bank.json').then(r=>r.json());officialQuestions=data.questions.filter(q=>q.review?.status!=='excluded'&&!q.needsImage&&q.options.length>=2).map(q=>{const answerIndex=q.answer.charCodeAt(0)-65;const status=q.review?.status==='verified'?'已按现行法规核验':'已自动排除明确旧规，待逐题人工复核';return{t:q.stem,a:q.options,c:answerIndex,e:`${q.explanation?`${q.explanation} `:''}来源：${q.source.name} · ${q.region} · ${status}`,topic:'本地完整题库',legacy:q.legacy,source:q.source,review:q.review}}).filter(q=>q.c>=0&&q.c<q.a.length);document.querySelector('#bankCount').textContent=questions.length+officialQuestions.length;const badge=document.querySelector('#bankStatus');if(badge)badge.textContent=`本地 ${officialQuestions.length} 题 · 已隔离 ${data.meta.excluded} 道疑似旧规`;if(quizMode==='official')startQuiz()}catch(e){console.warn('本地题库载入失败',e)}}
