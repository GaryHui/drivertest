...
function answer(choice){if(locked)return;locked=true;const q=quiz[qIndex],ok=choice===q.c,globalIndex=questions.indexOf(q);q._ok=ok;state.answered++;if(ok){state.correct++;if(globalIndex>=0)state.wrong=state.wrong.filter(i=>i!==globalIndex)}else if(globalIndex>=0&&!state.wrong.includes(globalIndex))state.wrong.push(globalIndex);save();quizBox.querySelectorAll('.answer').forEach((b,i)=>{if(i===q.c)b.classList.add('correct');else if(i===choice)b.classList.add('wrong');b.disabled=true});const memory=q.memory?`<div class="memory-tip">${q.memory}</div>`:'';const d=!ok&&q.breakdown?`<div class="wrong-breakdown"><p>先认场景，再找数字</p><div><span><small>谁</small>${q.breakdown.who}</span><span><small>发生什么</small>${q.breakdown.when}</span><span class="key"><small>答案数字</small>${q.breakdown.number}</span><span><small>上限／例外</small>${q.breakdown.limit}</span><span class="mix"><small>千万别选成</small>${q.breakdown.mix}</span></div>${memory}</div>`:'';feedback.innerHTML=`<div class="explain"><b>${ok?'✓ 判断正确':'✕ 这题混淆了'}</b><br>${q.e}</div>${ok?memory:d}<button class="next-btn">下一题 →</button>`;feedback.querySelector('button').onclick=()=>{qIndex++;renderQuestion()}}
