...
function answer(choice){if(locked)return;locked=true;const q=quiz[qIndex],ok=choice===q.c,globalIndex=questions.indexOf(q);q._ok=ok;state.answered++;if(ok){state.correct++;if(globalIndex>=0)state.wrong=state.wrong.filter(i=>i!==globalIndex)}else if(globalIndex>=0&&!state.wrong.includes(globalIndex))state.wrong.push(globalIndex);save();quizBox.querySelectorAll('.answer').forEach((b,i)=>{if(i===q.c)b.classList.add('correct');else if(i===choice)b.classList.add('wrong');b.disabled=true});const memory=q.memory?`<div class="memory-tip">${q.memory}</div>`:'';const movie=q.movieMemory?`<div class="movie-memory"><small>🎬 脑内短片：只看车动不动</small><b>车还在应急道上<strong>溜</strong>＝<strong>六</strong>分；车停住赖了很<strong>久</strong>＝<strong>九</strong>分。</b><div class="memory-pair"><span>🚗💨 应急道行驶<em>6</em></span><span>🚗🛑 高速违法停车<em>9</em></span></div><p>口诀只负责区分这两题：溜六，久九。</p></div>`:'';const d=!ok&&q.breakdown?`<div class="scene-card"><div class="scene-title"><span>🔍</span><div><small>场景密码</small><b>先抓人，再抓动作</b></div></div><div class="scene-clues"><span><small>谁</small>${q.breakdown.who}</span><i>＋</i><span><small>发生什么</small>${q.breakdown.when}</span></div><div class="scene-answer"><small>所以只选</small><strong>${q.breakdown.number}</strong><em>${q.breakdown.limit}</em></div><div class="scene-trap"><small>✕ 别被这个带跑</small><b>${q.breakdown.mix}</b></div>${memory}</div>`:'';feedback.innerHTML=`<div class="explain"><b>${ok?'✓ 判断正确':'✕ 这题混淆了'}</b><br>${q.e}</div>${movie}${ok?memory:d}<button class="next-btn">下一题 →</button>`;feedback.querySelector('button').onclick=()=>{qIndex++;renderQuestion()}}
