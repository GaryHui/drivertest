...
allCards.forEach((item,idx)=>{
  ...
  questions.push({
    ...
    breakdown:{who:item.card[1],when:item.card[2],number:item.card[3],limit:item.card[4],mix:item.card[5]}
  });
});
...
function answer(choice){if(locked)return;locked=true;const q=quiz[qIndex],ok=choice===q.c,globalIndex=questions.indexOf(q);q._ok=ok;state.answered++;if(ok){state.correct++;if(globalIndex>=0)state.wrong=state.wrong.filter(i=>i!==globalIndex)}else if(globalIndex>=0&&!state.wrong.includes(globalIndex))state.wrong.push(globalIndex);save();quizBox.querySelectorAll('.answer').forEach((b,i)=>{if(i===q.c)b.classList.add('correct');else if(i===choice)b.classList.add('wrong');b.disabled=true});const d=!ok&&q.breakdown?`<div class="wrong-breakdown"><p>别背整句，拆成这 5 格</p><div><span><small>谁</small>${q.breakdown.who}</span><span><small>什么情况</small>${q.breakdown.when}</span><span class="key"><small>关键数字</small>${q.breakdown.number}</span><span><small>上限／例外</small>${q.breakdown.limit}</span><span class="mix"><small>最容易混</small>${q.breakdown.mix}</span></div></div>`:'';feedback.innerHTML=`<div class="explain"><b>${ok?'✓ 判断正确':'✕ 这题混淆了'}</b><br>${q.e}</div>${d}<button class="next-btn">下一题 →</button>`;feedback.querySelector('button').onclick=()=>{qIndex++;renderQuestion()}}
