...
function startQuiz(){let pool=quizMode==='wrong'?questions.filter((_,i)=>state.wrong.includes(i)):quizMode==='numbers'?questions.filter(q=>q.number):quizMode==='newreg'?questions.filter(q=>q.newreg&&!q.generated):quizMode==='official'?officialQuestions:questions.filter(q=>!q.generated);if(!pool.length){quizBox.innerHTML=`<div class="empty"><h3>${quizMode==='official'?'旧题库核验中，暂不开放':'错题本还是空的 🎉'}</h3><p>${quizMode==='official'?'只让“法规原文＋当前题库”双重确认的题进入练习，宁可少题也不教错。':'先完成一轮摸底，答错的题会自动来到这里。'}</p></div>`;return}quiz=[...pool].sort(()=>Math.random()-.5).slice(0,10);qIndex=0;renderQuestion()}
...
async function loadOfficialBank(){try{const data=await fetch('data/question-bank.json').then(r=>r.json());officialQuestions=data.questions.filter(q=>q.category==='car-general'&&q.review?.status==='verified'&&!q.needsImage&&q.options.length>=2).map(q=>{const answerIndex=q.answer.charCodeAt(0)-65;return{t:q.stem,a:q.options,c:answerIndex,e:`${q.explanation?`${q.explanation} `:''}来源：${q.source.name} · ${q.region} · 已按现行法规核验`,topic:'已核验本地题库',legacy:q.legacy,source:q.source,review:q.review}}).filter(q=>q.c>=0&&q.c<q.a.length);document.querySelector('#bankCount').textContent=questions.length+officialQuestions.length;const badge=document.querySelector('#bankStatus');if(badge)badge.textContent=`已核验 ${officialQuestions.length} 题 · 待核验题暂不开放`;if(quizMode==='official')startQuiz()}catch(e){console.warn('本地题库载入失败',e)}}
