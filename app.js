...
const naturalStems={'信息变更':'驾驶人的联系电话、联系地址等信息发生变化，应当在多少日内办理备案？','换证窗口':'机动车驾驶证有效期即将届满，最早可以提前多少日申请换证？','实习期':'初次取得汽车类驾驶证后的实习期是多久？','满分学习·小型车':'小型汽车驾驶人每多一次满分教育或累计记分每增加12分，学习增加几天？','满分学习·重点车':'重点大车驾驶人每多一次满分教育或累计记分每增加12分，学习增加几天？','驾驶证审验':'大中型客货车驾驶人在一个记分周期内有记分，应在周期结束后多久内审验？','学习驾驶证明':'科目一合格后取得的学习驾驶证明，有效期是多久？'};
...
allCards.forEach((item,idx)=>{
  ...
  questions.push({
    t:naturalStems[item.card[0]]||`关于“${item.card[0]}”，正确的数字或处理结果是？`,a:answers,c:correctAt,
    e:`${item.card[1]}，${item.card[2]}：${correct}。易混提醒：${item.card[5]}。`,
    topic:item.topic,number:/\d/.test(correct),newreg:['记分题','时间题','证件题'].includes(item.topic),rule:`${topics.find(t=>t.name===item.topic).id}-${item.index}`,
