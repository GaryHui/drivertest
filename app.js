...
{t:'驾驶机动车在高速公路或者城市快速路上违法停车，一次记几分？',a:['3分','6分','9分','12分'],c:2,e:'高速或城市快速路违法停车记9分。占应急车道行驶才是6分。',topic:'新规题',newreg:true,number:true,breakdown:{who:'机动车驾驶人',when:'高速公路或城市快速路违法停车',number:'9分',limit:'不分是否停在应急车道',mix:'占应急车道行驶是6分'}},
...
let currentTopic=topics[0].id,quizMode='mixed',quiz=[],qIndex=0,locked=false,officialQuestions=[],onlineQuestions=[],onlineLoading=false;
...
function startQuiz(){if(quizMode==='online'&&!onlineQuestions.length){loadOnlineBank();return}let pool=quizMode==='wrong'?questions.filter((_,i)=>state.wrong.includes(i)):quizMode==='numbers'?questions.filter(q=>q.number):quizMode==='newreg'?questions.filter(q=>q.newreg&&!q.generated):quizMode==='official'?officialQuestions:quizMode==='online'?onlineQuestions:questions.filter(q=>!q.generated);if(!pool.length){quizBox.innerHTML=`<div class="empty"><h3>${quizMode==='official'?'题库正在载入':'错题本还是空的 🎉'}</h3><p>${quizMode==='official'?'请稍候再试。':'先完成一轮摸底，答错的题会自动来到这里。'}</p></div>`;return}quiz=[...pool].sort(()=>Math.random()-.5).slice(0,10);qIndex=0;renderQuestion()}
...
function answer(choice){if(locked)return;locked=true;const q=quiz[qIndex],ok=choice===q.c,globalIndex=questions.indexOf(q);q._ok=ok;state.answered++;if(ok){state.correct++;if(globalIndex>=0)state.wrong=state.wrong.filter(i=>i!==globalIndex)}else if(globalIndex>=0&&!state.wrong.includes(globalIndex))state.wrong.push(globalIndex);save();quizBox.querySelectorAll('.answer').forEach((b,i)=>{if(i===q.c)b.classList.add('correct');else if(i===choice)b.classList.add('wrong');b.disabled=true});const memory=q.memory?`<div class="memory-tip">${q.memory}</div>`:'';const d=!ok&&q.breakdown?`<div class="scene-card"><div class="scene-title"><span>🔍</span><div><small>场景密码</small><b>先抓人，再抓动作</b></div></div><div class="scene-clues"><span><small>谁</small>${q.breakdown.who}</span><i>＋</i><span><small>发生什么</small>${q.breakdown.when}</span></div><div class="scene-answer"><small>所以只选</small><strong>${q.breakdown.number}</strong><em>${q.breakdown.limit}</em></div><div class="scene-trap"><small>✕ 别被这个带跑</small><b>${q.breakdown.mix}</b></div>${memory}</div>`:'';feedback.innerHTML=`<div class="explain"><b>${ok?'✓ 判断正确':'✕ 这题混淆了'}</b><br>${q.e}</div>${ok?memory:d}<button class="next-btn">下一题 →</button>`;feedback.querySelector('button').onclick=()=>{qIndex++;renderQuestion()}}
...
async function loadOnlineBank(){if(onlineLoading)return;onlineLoading=true;quizBox.innerHTML='<div class="empty"><h3>正在连接在线题库…</h3><p>首次加载可能需要几秒钟。</p></div>';try{const r=await fetch('/api/questions?model=c1&type=rand'),data=await r.json();if(!r.ok)throw new Error(data.message||'载入失败');onlineQuestions=(data.questions||[]).map(q=>{const opts=[q.item1,q.item2,q.item3,q.item4].filter(Boolean),index=Number(q.answer)-1;return{t:q.question,a:opts.length?opts:['正确','错误'],c:index,e:q.explains||'暂无解析',topic:'在线新题库',image:q.url||''}}).filter(q=>q.c>=0&&q.c<q.a.length);if(quizMode==='online')startQuiz()}catch(e){quizBox.innerHTML=`<div class="empty"><h3>在线题库暂未启用</h3><p>${e.message}</p><p>需要在 Vercel 配置 JUHE_KEY。</p></div>`}finally{onlineLoading=false}}
