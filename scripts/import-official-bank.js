...
const questions=[];let q=null,part='stem',category='car-general';
...
for(const line of lines){if(/客车专用试题/.test(line)){finish();category='bus-special';continue}if(/货车专用试题/.test(line)){finish();category='truck-special';continue}if(/轮式(专用|自行)机械车.*试题/.test(line)){finish();category='machinery-special';continue}const s=line.match(start);if(s){finish();q={id:s[1],stem:s[2],options:[],answer:'',category,source:'承德市公安局2025-08-20公开汽车类题库（原文档为旧版题库）'};part='stem';continue}if(!q)continue;const a=line.match(ans);if(a){q.answer=a[1].toUpperCase();continue}const o=line.match(opt);if(o){q.options.push(o[2]);part='option';continue}if(part==='stem'&&!/^\d+(\.\d+){1,2}/.test(line))q.stem+=' '+line;else if(part==='option'&&q.options.length)q.options[q.options.length-1]+=' '+line}
